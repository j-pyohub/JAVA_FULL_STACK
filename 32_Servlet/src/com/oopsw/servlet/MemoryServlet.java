...
public class MemoryServlet extends HttpServlet {
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//cmd의 값을 구분해서 메모리에 저장
		request.setAttribute("r1", cmd);
		HttpSession session = request.getSession(true); //매개인자o -> 확실히 만듦. 로그인, 쇼핑몰에서 오늘 본 상품 정보 같은 업무
		session.setAttribute("s1", "session " + cmd);
		ServletContext application = request.getServletContext();
		if (application.getAttribute("info") == null)
			application.setAttribute("info", "공유데이터"); //ex. 로그인한 사람 이름: value에 collection으로 넣을 수 있음. 
